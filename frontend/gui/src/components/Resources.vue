...
<script>
  export default {
    ...
    dialog: false,
      ...
      { text: 'Name', align: 'start', sortable: false, value: 'resource_name' },
  ...
  computed: {
     ...
     */
    formTitle () {
      return this.editedObjectIndex === -1 ? 'New Resource' : 'Edit Tag'
    },
    date () {
        var today = new Date()
  ...
  await fetch(
        'http://localhost:5000/droots/resources'
        )
        .then(response => {
        ...
        .then(data => {
          ...
          this.ObjectsList = data.Resources_Available
        })
        return
    },
    validate: function () {
      ...
      if (this.editedObjectIndex > -1) {
        ...
        await fetch(
          ...
          .then(data => {
            ...
            this.ObjectsList.push(data.Resources_Available)
          })
          .catch((error) => {
            console.error('There has been a problem with your fetch operation:', error)
          })
