            ...
            >
              <template v-slot:top>
                <v-toolbar flat color="white">
                  <v-toolbar-title>Water</v-toolbar-title>
                  <v-divider
                    class="mx-4"
                    inset
                    vertical
                  ...
                  <v-dialog persistent v-model="dialog" max-width="400px">
                    <template v-slot:activator="{ on }">
                      <v-btn color="#39b54a" dark class="mb-2" v-on="on">Add Water</v-btn>
                    </template>
                    <v-card >
                      <v-card-title>
                      ...
                      <v-card-text>
                        <v-container>
                          <v-row>
                            <v-col cols="6">
                              <v-text-field v-model="editedObject.water_name"
                              label="Name"
                              :counter="10"
                            ...
                            <v-col cols="6">
                              <v-select
                                v-model="editedObject.water_type"
                              :rules="[ v => !!v || 'type  is required']"
                                :items="['Filtered', 'Distilled']"
                                label="Type"
                                flat >
                            </v-select>
                            </v-col>
                            <v-col cols="12">
                              <v-text-field v-model="editedObject.water_oz"
                              label="oz (per unit)"
                              :counter="5"
                              :rules="[ v => !!v || 'description is required', v => (v && v.length <= 5) || 'description must be less than 250 characters' ]"
                              ></v-text-field>
                            </v-col>
                          </v-row>
                        </v-container>
...
<script>
  export default {
    ...
    dialog: false,
      ...
      { text: 'Name', align: 'start', sortable: false, value: 'water_name' },
      { text: 'Type', align: 'start', sortable: false, value: 'water_type' },
      { text: 'oz (per unit)', align: 'start', sortable: false, value: 'water_oz' },
  ...
  computed: {
     ...
     */
    formTitle () {
      return this.editedObjectIndex === -1 ? 'New Water' : 'Edit Tag'
    },
    date () {
        var today = new Date()
  ...
  await fetch(
        'http://localhost:5000/droots/resources/water'
        )
        .then(response => {
    ...
    save: async function () {
      if (this.editedObjectIndex > -1) {
        ...
        this.editedObject.water_date = this.date
        this.editedObject.water_oz = parseInt(this.editedObject.water_oz,10)
        await fetch(
          'http://localhost:5000/droots/resources/water',
          {
            method: 'POST',
          ...
          .then(data => {
            ...
            this.ObjectsList.push(data.Water)
          })
          .catch((error) => {
            console.error('There has been a problem with your fetch operation:', error)
          })
